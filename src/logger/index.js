...
const logger = function(config) {
  ...
  let accessLogFile = path.normalize(path.resolve( config.logger.filename));//path.join(path.dirname(process.mainModule.filename), config.logger.filename);

  // create a write stream (in append mode)
	let stream = fs.createWriteStream(/*config.logger.fileName ||*/ accessLogFile, {
		'flags': 'a+',
		'encoding': 'utf8'
	});
