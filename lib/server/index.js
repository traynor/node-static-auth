  ...
  _createClass(NodeStatic, [{
    ...
    value: function createServer() {
      ...
      this.server.listen(this.config.server.port, function () {
        ...
        console.log('Node-static-auth ' + (_this.supportsHttp2 ? 'HTTP/2 ' : '') + (_this.sslOpts ? 'secure ' : 'unsecure ') + 'server running on port ' + _this.config.server.port);
    ...
    value: function listener(request, response) {
      ...
      this.logger.log(request, response, function (next) {

        if (_this2.config.auth.enabled) {

          var credentials = (0, _basicAuth2.default)(request);

          if (!credentials || credentials.name !== _this2.config.auth.name && credentials.pass !== _this2.config.auth.pass) {
