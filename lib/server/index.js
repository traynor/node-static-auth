...
var NodeStatic = function () {
  function NodeStatic(inputConfig) {
    ...
    this.config = Object.assign(_defaultConfig2.default, inputConfig);
    this.cb = cb;
    this.logger = new _logger2.default(this.config);
    ...
    this.fileServer = new _nodeStatic2.default.Server(this.config.nodeStatic.root);
  ...
  _createClass(NodeStatic, [{
    ...
    value: function createServer() {
      ...
      this.server.listen(this.config.server.port, function () {
        ...
        if (_this.cb) _this.cb(_this.server);
      });
      ...
      if (this.sslOpts) {
        ...
        _http3.default.createServer(function (request, response) {
          ...
          _this.logger.log(request, response, function (next) {
            console.log('http listener redirecting', _this.sslOpts && !/https/.test(request.protocol), request.url, request.headers.host);
    ...
    value: function listener(request, response) {
      ...
      var hostHeader = this.supportsHttp2 ? request.headers[':authority'] : request.headers.host;
      ...
      if (this.config.auth.enabled) {
        ...
        if (!credentials || credentials.name !== this.config.auth.name && credentials.pass !== this.config.auth.pass) {
          if (this.config.server.customPages && !this.config.server.http2) {
            _utils2.default.sendCustom(request, response, 401, this.config.server.customPages.forbidden, this.fileServer, this.logger.log.bind(this.logger));
          } else {
        ...
        this.fileServer.serve(request, response, function (err, result) {
          ...
          if (err) {
            ...
            if (err.status === 404) {
              ...
              _utils2.default.sendCustom(request, response, 404, _this2.config.server.customPages.notFound, _this2.fileServer, _this2.logger.log.bind(_this2.logger));
            } else {

              _utils2.default.sendCustom(request, response, 500, _this2.config.server.customPages.error, _this2.fileServer, _this2.logger.log.bind(_this2.logger));
            }
          } else {
