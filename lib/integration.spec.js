...
before(function (done) {
  ...
  var nodeStaticAuth = new NodeStaticAuth(config, function (svr, log) {
    inst = svr;
...
describe('static-auth server', function () {
  ...
  if (config.server.http2 && supportsHttp2) {
    ...
    it('should login via Basic auth and access server', function (done) {
      ...
      _superagent2.default.get((config.server.ssl.enabled ? 'https://' : 'http://') + 'localhost:' + config.server.port + '/?successful-login').auth('test', 'test', {
        type: 'auto'
    ...
    it('should get other static files', function (done) {
      ...
      _superagent2.default.get((config.server.ssl.enabled ? 'https://' : 'http://') + 'localhost:' + config.server.port + '/css.css').auth('test', 'test', {
        type: 'auto'
      }).ca(cert).end(function (err, res) {
        (0, _betterAssert2.default)(res.ok);
        logs.push('/css.css');
